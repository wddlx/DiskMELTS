...
These tools represent the current state of the art, but even accelerated slab-model fitting can require tens of minutes to several hours per source when the $(T, N, A)$ parameter space is explored thoroughly, and potentially days for full Bayesian posterior sampling (DD: how many CPU hours? I assume you are using HPC with multiple cores?). For JWST surveys targeting tens to hundreds of disks, this cost becomes a major bottleneck, especially when multiple molecules must be fitted and subtracted sequentially.

`DiskMELTS` (DD: add Full name here) addresses this bottleneck by replacing the radiative-transfer forward model with pretrained neural-network surrogates. A complete retrieval for one molecule in one source typically runs in under one minute on a standard laptop CPU, more than 1000 times faster than conventional approaches while preserving accuracy comparable to classical LTE slab-model fits. 

(DD: I feel there is no need to discuss the model grid here, we can just discuss this later) The default distribution provides surrogate models for H$_2$O, C$_2$H$_2$, HCN, and CO$_2$ over the $11$--$19\,\mu$m MIRI range, covering $T = 100$--$1400$ K and $\log_{10}(N/{\mathrm{cm}}^{-2}) = 13$--$19$.
