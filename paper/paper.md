...
Machine-learning Enabled Line fitting Tool for Spectra (`DiskMELTS`) addresses this bottleneck by replacing the radiative-transfer forward model with pretrained neural-network surrogates. A complete retrieval for one molecule in one source typically runs in under one minute on a standard laptop CPU, more than three orders of magnitude faster than conventional approaches while preserving accuracy comparable to classical LTE slab-model fits. 
...
The `DiskMELTS` framework is implemented across four Python modules designed for spectral retrieval and model customization. The core workflow relies on `fitting` to execute retrievals using pretrained models and `plotting` to generate diagnostic figures. For advanced applications such as training custom models across alternative wavelengths or parameter spaces, the package provides `trainmodel` for surrogate training and slab-grid utilities, alongside `validation` for synthetic recovery tests. The default pipeline initiates with precomputed Local Thermodynamic Equilibrium (LTE) slab spectra to train compact neural surrogates for individual molecular species, which are subsequently deployed within a global-to-local optimization routine to fit observed spectra.
...
Each precomputed grid point is stored as a CSV file named by its temperature and column density, and the training utilities parse those filenames to recover $(T,\log_{10}N)$. The default pretrain set and models are generated assuming all the line broadening are purely thermal. For C-bearing molecules, only the $12$--$16.5\,\mu$m region is passed to the training pipeline, excluding long-wavelength channels with little signal for these species. H$_2$O uses the full $11$--$19\,\mu$m range.
