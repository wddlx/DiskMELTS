...
Extracting physical parameters from these spectra commonly requires fitting local thermodynamic equilibrium (LTE) slab models to the observed molecular emission. The key parameters for each molecule are the excitation temperature $T$, molecular column density $N$, and emitting area $A$. Several tools are already available for this task. CLIcK [@Liu19] provides a grid-based slab-model fitting approach, IRIS [@Romero24] implements JAX-accelerated line-by-line radiative transfer and supports Bayesian retrieval with `dynesty`, and `DuCKLinG` [@Kaeufer24] combines slab-model fitting with dust models based on pre-computed dust and gas models, also with Bayesian retrieval and linear regression. 

These tools represent the current state of the art, but even accelerated slab-model fitting can require tens of minutes to several hours per molecule when the $(T, N, A)$ parameter space is explored thoroughly, and potentially days per source with GPU accelerating. For JWST surveys targeting tens to hundreds of disks, this cost becomes a major bottleneck, especially when multiple molecules must be fitted and subtracted sequentially.

Machine-learning Enabled Line fitting Tool for (Disk) Spectra (`DiskMELTS`) addresses this bottleneck by replacing the radiative-transfer forward model with pretrained neural-network surrogates. A complete retrieval for one molecule in one source typically runs in under one minute on a standard laptop CPU, more than three orders of magnitude faster than conventional approaches while preserving accuracy comparable to classical LTE slab-model fits. 
...
The `DiskMELTS` framework is implemented across four Python modules designed for spectral retrieval and model customization for continuum-subtracted spectra [^1]. The core workflow relies on `fitting` to execute retrievals using pretrained models and `plotting` to generate diagnostic figures. For advanced applications such as training custom models across alternative wavelengths or parameter spaces, the package provides `trainmodel` for surrogate training and slab-grid utilities, alongside `validation` for synthetic recovery tests. The default pipeline initiates with precomputed Local Thermodynamic Equilibrium (LTE) slab spectra to train compact neural surrogates for individual molecular species, which are subsequently deployed within a global-to-local optimization routine to fit observed spectra. 
...
Each precomputed grid point is stored as a CSV file named by its temperature and column density, and the training utilities parse those filenames to recover $(T,\log_{10}N)$. The default pretrain set and models are generated assuming all the line broadening are purely thermal. 

| Molecule | Components | Variance / Accuracy (%) | Wavelength Range (µm) |     |
| :------- | :--------: | :---------------------: | :-------------------: | --- |
| H₂O      |     21     |         99.992%         |      11.0 – 19.0      |     |
| C₂H₂     |     15     |         99.990%         |      12.0 – 16.5      |     |
| HCN      |     15     |         99.980%         |      12.0 – 16.5      |     |
| CO₂      |     15     |         99.988%         |      12.0 – 16.5      |     |

[//]: # (DD: I think here we can probably have a table to show the parameter ranges and wavelength coverage for each molecule, and maybe also the number of grid points. We can also mention that the grid is generated with a custom code that we can make public if there is interest, but I don't think we need to go into details about the grid generation here. Also we should say that there is an option to train on a custom grid, so users can generate their own grids with their own code and then feed them into our training pipeline.)
...
When applied to real JWST/MIRI spectra, DiskMELTS yields molecular parameters consistent with classical LTE slab-model fitting, typically agreeing within $\sim 0.5$ dex for $N \times A$ and $< 150$ K for $T$ relative to literature benchmarks [@Romero24; Xie et al. in prep.; Raul et al. in prep.]. As with standard slab retrievals, $N$ and $A$ can exhibit strong degeneracies, particularly for optically thin emissions; however, their product $N \times A$ (which traces the total number of emitting molecules) is more robustly recovered [^2]. For blended C₂H₂ and HCN emission, multiple parameter combinations can produce comparably good fits, meaning the returned uncertainties encompass both inherent slab-model degeneracies and surrogate-model approximation errors.

[^2]: These tests can be seen in figures/realobs_validations/. Two of the real observation data are provided, and the rest will be added after the two papers in preparation are published. 

`DiskMELTS` is not limited to the four molecules and two isotopes included in the default distribution. The `trainmodel` module exposes the full training pipeline: grid loading, pretrain CSV generation, and two-MLP training, so that users with their own slab model grids can train surrogate models for any molecule and integrate them directly into the fitting workflow.
